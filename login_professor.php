...
##########################################################################################
		...
		if (!$user || !$senha) {
			...
			header("Location: login_professor.php?erro=nada");
		}else{

			$lista = $conexao->prepare("select * from professor where nome = :user LIMIT 1");
			...
			if($lista->rowCount() > 0){
				...
				if (password_verify($senha, $usuario->senha)) {
						...
						header("Location: login_professor.php?erro=senhaerrado");
					}

			}else{
				header("Location: login_professor.php?erro=usererrado");
			}
